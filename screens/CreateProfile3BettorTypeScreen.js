...
import { ButtonOutline, Icon, RadioButtonGroup, RadioButtonRow, ScreenContainer, Surface, SwitchRow, Touchable, withTheme } from '@draftbit/ui';
import { StatusBar, StyleSheet, Text, View } from 'react-native';
  ...
  StatusBar.setBarStyle('light-content');
      ...
      const [radioButtonGroupBettorType, setRadioButtonGroupBettorType] = React.useState(undefined);
const [sellPicksSwitch, setSellPicksSwitch] = React.useState(false);
    ...
    <View  style={styles.ViewY1}  /><View  style={[styles.ViewYM, { borderRadius: 0 }]}  />
      </View>
     ...
     (newRadioButtonGroupValue) => { const radioButtonGroupBettorType = newRadioButtonGroupValue;
      try {
  ...
  } style={styles.RadioButtonGroup_30}  direction={"vertical"} defaultValue={radioButtonGroupBettorType}>
        <RadioButtonRow  style={[styles.RadioButtonRowXF, { color: theme.colors.background_inverse }]} label={"New bettor"} direction={"row"} /><RadioButtonRow  style={[styles.RadioButtonRowmr, { color: theme.colors.background_inverse }]} label={"Occasional bettor"} direction={"row"} /><RadioButtonRow  style={[styles.RadioButtonRowRN, { color: theme.colors.background_inverse }]} label={"Avid bettor"} direction={"row"} /><RadioButtonRow  style={[styles.RadioButtonRowBX, { color: theme.colors.background_inverse }]} label={"Professional bettor"} direction={"row"} />
      </RadioButtonGroup>
    <SwitchRow onValueChange={
     (newSwitchRowValue) => { const sellPicksSwitch = newSwitchRowValue;
      try {
  ...
  } style={[styles.SwitchRowqf, { color: theme.colors.background_inverse }]} label={"I sell my picks"} value={sellPicksSwitch} />
  ...
  } style={[styles.ButtonOutlineeh, { color: theme.colors.background, backgroundColor: theme.colors.primary }]} title={"Next"}  />
      </View>
...
const styles = StyleSheet.create({ ViewxY: {
