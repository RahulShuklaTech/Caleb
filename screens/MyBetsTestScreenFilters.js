...
import { ButtonOutline, ButtonSolid, DatePicker, ScreenContainer, Touchable, withTheme } from '@draftbit/ui';
...
let array = [...Constants.filterLeague,...Constants.filterDate,...Constants.filterSportsbook]
...
const [loadingReset, setLoadingReset] = React.useState(false);
 ...
 console.log((Constants["combinedFiltersArray"]).toString())
  ...
  } style={styles.ButtonOutline_8B} title={"Get Started"} /><FlatList data={Constants["combinedFiltersArray"]} renderItem={({ item }) => { const listData = item; return (<>{ !(listData) ? null : 
      <View  style={[styles.ViewzV, { backgroundColor: theme.colors.primary }]} pointerEvents={"auto"}>
     ...
     }</>) }}  contentContainerStyle={styles.FlatListrZContent} numColumns={1} />
      <Text  style={{ color: theme.colors.strong }} >
      ...
      <Modal  style={[styles.Modal_5J, { backgroundColor: theme.colors.background, borderColor: theme.colors.background }]} animationType={"slide"} presentationStyle={"pageSheet"} transparent={false}>
        <DatePicker onDateChange={
     (newDatePickerValue) => { const date = newDatePickerValue;
      try {
          setDate(date);
      } catch (err) {
        console.error(err)
  ...
  } style={styles.DatePickerZt} label={"Date"} mode={"date"} leftIconMode={"inset"} type={"solid"} date={date} />
      </Modal>
...
const styles = StyleSheet.create({ ButtonSolidMc: {
borderRadius: 8,
