...
const setGlobalVariableValue = GlobalVariables.useSetValue();
  ...
  StatusBar.setBarStyle('light-content');
      ...
      <View  style={styles.View_2T} >
        <Icon   name={"Ionicons/ios-chevron-back-sharp"} size={28} color={theme.colors.background} />
      </View>
    
      </Touchable>
    
      </View>
    <View  style={styles.ViewzX}  /><View  style={[styles.ViewMt, { borderRadius: 0 }]}  />
      </View>
      ...
      <Text  style={[styles.Textlc, { color: theme.colors.background_inverse }]} >
        {"Welcome to Vault 👋\n\nLet's customize your profile!"}
      </Text>
    
      <Text  style={[styles.TextCh, { color: theme.colors.background_inverse }]} >
        {"What is your typical unit size?"}
      </Text>
    
      <View  style={styles.ViewTN} >
        
      <KeyboardAvoidingView  style={styles.KeyboardAvoidingViewKZ} enabled={true} behavior={"padding"}>
        <TextField onChangeText={
     (newTextFieldValue) => { const userDefaultUnitSize = newTextFieldValue;
      try {
  ...
  setGlobalVariableValue({
  ...
  } style={[styles.TextFieldh8, { color: theme.colors.background_inverse, borderRadius: 0 }]} error={userDefaultUnitSizeError(userDefaultUnitSize)} placeholder={"Must be greater or equal to 1"} type={"solid"} value={userDefaultUnitSize} autoFocus={true} placeholderTextColor={theme.colors.light} defaultValue={"100"} leftIconMode={"outset"} leftIconName={"MaterialIcons/attach-money"} editable={true} keyboardType={"number-pad"} returnKeyLabel={"Next"} returnKeyType={"next"} />
    ...
    async () => { 
      try {
         if (userDefaultUnitSizeError(userDefaultUnitSize)) { return }
  navigation.navigate('CreateProfileBetaStack',
{ screen: 'CreateProfile5ChooseYourPromoScreen'});
 await SwaggerAPIApi.saveUnitSizeGET(Constants, { unitSize: userDefaultUnitSize,});
      } catch (err) {
        console.error(err)
      }
    }
  } style={[styles.ButtonOutline_1d, { color: theme.colors.strong, backgroundColor: theme.colors.primary }]} title={"Next"}  />
      </View>
...
const styles = StyleSheet.create({ View_2T: {
