...
const m = diffSec / 60
const s = diffSec
if(d >= 1){return parseInt(d) + "d"}
if(h >= 1){return parseInt(h) + "h"}
if(m >= 1){return parseInt(m) + "m"}
if(s > 0){return parseInt(s) + "s"}
}else{
return 0 + "s"
}
  };
...
const [textInputValue, setTextInputValue] = React.useState("");
  ...
  setDummy(addOne(dummy));
      ...
      <View  style={styles.ViewqG} >
        <Icon   name={"Ionicons/ios-add-circle"} size={28} color={theme.colors.light} />
      </View>
      ...
      <View  style={styles.ViewFd} >
        <Image  style={styles.ImageCE} resizeMode={"contain"} source={Images.VaultLogoLightFontClearBackground} />
      </View>
    ...
    <>{ !(Constants["waitlisted"]) ? null : 
      ...
      <View  style={[styles.ViewP9, { backgroundColor: theme.colors.primary }]} pointerEvents={"auto"}>
...
<SwaggerAPIApi.FetchGetBankrollPageByIdGET
 ...
 >
  ...
  } style={[styles.TextInputar, { borderColor: theme.colors.divider, color: theme.colors.strong }]} value={textInputValue} placeholder={"Early Access Code..."} placeholderTextColor={theme.colors.divider} /><IconButton onPress={
     () => { 
      try {
  ...
  setGlobalVariableValue({
  ...
  } style={styles.IconButtonR2} icon={"Ionicons/ios-arrow-forward-circle"} size={32} color={theme.colors.strong} />
      </View>
      ...
      <ScrollView   contentContainerStyle={styles.ScrollViewEpContent} showsVerticalScrollIndicator={true} bounces={true}>
        
      <View  style={styles.View_8S} pointerEvents={"auto"}>
        <Image  style={styles.ImageEC} source={Images.MoneyDemo} resizeMode={"contain"} />
      </View>
     ...
     () => { 
      ...
      <View  style={[styles.Viewz0, { backgroundColor: theme.colors.primary }]} pointerEvents={"auto"}>
        <Icon  style={styles.IconCz} name={"Ionicons/ios-add-circle"} size={28} />
      <View  style={styles.ViewJ8} pointerEvents={"auto"}>
        
      <Text  style={[styles.TextKh, { color: theme.colors.strong }]} >
        {"You're In Demo Mode"}
      </Text>
    
      <Text  style={[styles.TextK0, { color: theme.colors.strong }]} >
      ...
      <ScrollView   contentContainerStyle={styles.ScrollViewaWContent} showsVerticalScrollIndicator={true} bounces={true}>
        
      <View  style={styles.VieweZ} pointerEvents={"auto"}>
        <Image  style={styles.ImagefK} source={Images.MoneyDemo} resizeMode={"contain"} />
      </View>
...
<SwaggerAPIApi.FetchGetBankrollPageByIdGET
 ...
 >
  {({ loading, error, data, refetchGetBankrollPageById }) => {
    ...
    return (
      ...
      <View  style={styles.ViewIU} >
        <FlatList data={checkSportsbooksSynced(fetchData)} renderItem={({ item }) => { const listData = item; return (<>{ !(userSelectedSportsbook(listData?.account?.id)) ? null : 
     ...
     () => { 
      ...
      <Text  style={[styles.Text_8T, { color: theme.colors.light }]} >
        {listData?.account?.book?.name}{bookRegionSyntax(listData?.account?.bookRegion?.abbr)}{" Bankroll"}
      </Text>
    <Icon  style={styles.IconOY} name={"Ionicons/ios-chevron-down"} color={theme.colors.light} size={16} />
      </View>
    
      <View  style={styles.ViewG2} pointerEvents={"auto"}>
        
      <Text  style={[styles.TextBL, { color: theme.colors.background_inverse }]} >
    ...
    <>{ !(listData?.pending) ? null : 
      <Text  style={[styles.Textbn, { color: theme.colors.background_inverse }]} >
        {"("}{balanceSyntax(listData?.pending)}{" Pending)"}
      </Text>
     }</>
      </View>
     ...
     () => { 
      ...
      <Text  style={[styles.TextPX, { color: theme.colors.light }]} >
        {listData?.account?.book?.name}{bookRegionSyntax(listData?.account?.bookRegion?.abbr)}{" Net Profit"}
      </Text>
    <Icon  style={styles.IconZD} name={"Ionicons/ios-chevron-down"} color={theme.colors.light} size={16} />
      </View>
    <>{ !(valuePositive(listData?.netProfit)) ? null : 
      <Text  style={[styles.Textry, { color: theme.colors.good }]} >
        {balanceSyntax(listData?.netProfit)}
      </Text>
     }</><>{ !(valueNegative(listData?.netProfit)) ? null : 
      <Text  style={[styles.TextJC, { color: theme.colors.error }]} >
        {balanceSyntax(listData?.netProfit)}
      </Text>
     }</><>{ !(valueNeutral(listData?.netProfit)) ? null : 
      <Text  style={[styles.Text_12, { color: theme.colors.background_inverse }]} >
        {balanceSyntax(listData?.netProfit)}
      </Text>
     }</>
      </Touchable>
     ...
     }</>) }}  contentContainerStyle={styles.FlatList_0OContent} numColumns={1} />
      <View  style={styles.ViewfP} pointerEvents={"auto"}>
        
      <View  style={styles.Viewm4} >
        <Utils.CustomCodeErrorBoundary><CustomCode.VictoryLineNetProfit chartData = {fetchData}/></Utils.CustomCodeErrorBoundary>
      </View>
      ...
      <View  style={styles.ViewbY} >
        <FlatList data={fetchData} renderItem={({ item }) => { const listData = item; return (
  ...
  setGlobalVariableValue({
      ...
      <Surface  style={[styles.Surface_9A, { borderRadius: 6, backgroundColor: theme.colors.background }]} elevation={1}>
        
      <View  style={[styles.ViewAa, { backgroundColor: theme.colors.divider, borderRadius: 6, borderColor: theme.colors.divider }]} collapsable={false}>
      ...
      <Text  style={[styles.TextB5, { color: theme.colors.background_inverse }]} >
        {listData?.account?.book?.name}{bookRegionSyntax(listData?.account?.bookRegion?.abbr)}
      </Text>
    <>{ !(userSelectedSportsbook(listData?.account?.id)) ? null : <Icon  style={styles.IcontB} name={"MaterialIcons/stop-circle"} color={theme.colors.primary} size={10} /> }</>
      </View>
    <>{ !(listData?.account?.betRefreshRequested) ? null : 
      <View   pointerEvents={"auto"}>
        <>{ promptResyncBook(listData?.account) ? null : 
      <Text  style={[styles.TextWg, { color: theme.colors.light }]} >
        {"Refreshed "}{dateRefreshed(listData?.account?.betRefreshRequested)}{" ago"}
      </Text>
     }</><>{ !(promptResyncBook(listData?.account)) ? null : 
      <View  style={styles.Viewrx} pointerEvents={"auto"}>
        
      <Text  style={[styles.Textpj, { color: theme.colors.light }]} >
        {"Connection error. Press"}
      </Text>
    <Icon  style={styles.Iconu0} name={"Ionicons/ios-add-circle"} size={10} color={theme.colors.light} />
      <Text  style={[styles.TextHE, { color: theme.colors.light }]} >
        {"to resync book"}
      </Text>
      ...
      <View  style={styles.ViewoQ} pointerEvents={"auto"}>
        <>{ !(valuePositive(listData?.netProfit)) ? null : 
      <Text  style={[styles.Textri, { color: theme.colors.good }]} >
        {balanceSyntax(listData?.netProfit)}
      </Text>
     }</><>{ !(valueNegative(listData?.netProfit)) ? null : 
      ...
      <Text  style={[styles.TextR2, { color: theme.colors.background_inverse }]} >
        {balanceSyntax(listData?.netProfit)}
      </Text>
     }</><>{ !(aggregateCompare(listData?.account?.id)) ? null : 
      <View   pointerEvents={"auto"}>
        <>{ !(valuePositive(listData?.netProfit)) ? null : 
      <Text  style={[styles.Text_8g, { color: theme.colors.good }]} >
        {"Net Profit"}
      </Text>
     }</><>{ !(valueNegative(listData?.netProfit)) ? null : 
      <Text  style={[styles.TextCO, { color: theme.colors.error }]} >
        {"Net Profit"}
      </Text>
     }</><>{ !(valueNeutral(listData?.netProfit)) ? null : 
      <Text  style={[styles.TextBT, { color: theme.colors.background_inverse }]} >
        {"Net Profit"}
      </Text>
     }</>
      </View>
     }</><>{ !(valuePositive(listData?.winnings)) ? null : 
      <Text  style={[styles.TextbY, { color: theme.colors.good }]} >
        {"+"}{balanceSyntax(listData?.winnings)}
      </Text>
     }</><>{ !(valueNeutral(listData?.winnings)) ? null : 
      <Text  style={[styles.TextnH, { color: theme.colors.background_inverse }]} >
        {balanceSyntax(listData?.winnings)}
      </Text>
     }</><>{ !(aggregateCompare(listData?.account?.id)) ? null : 
      <View   pointerEvents={"auto"}>
        <>{ !(valuePositive(listData?.winnings)) ? null : 
      <Text  style={[styles.TexteH, { color: theme.colors.good }]} >
        {"Winnings"}
      </Text>
     }</><>{ !(valueNeutral(listData?.winnings)) ? null : 
      <Text  style={[styles.TextIB, { color: theme.colors.background_inverse }]} >
        {"Winnings"}
      </Text>
     }</>
      </View>
    ...
    ) }}  contentContainerStyle={styles.FlatList_5bContent} numColumns={1} />
      </View>
  ...
  } style={[styles.ButtonOutlineBA, { borderRadius: 6 }]} title={"Sync A Sportsbook"} icon={"Ionicons/ios-add-circle"}  loading={false} />
      </View>
...
const styles = StyleSheet.create({ ViewqG: {
