  ...
  const filterBetslipApplied = betslip => {
    ...
    function loopLeague() {
      ...
      for (let x = 0; x < betslip.bets.length; x++) {
        if (
          betslip.bets[x].event === null ||
          betslip.bets[x].event.league === null
        ) {
          exist;
        } else if (
          Constants.filterLeagueApply.includes(betslip.bets[x].event.league)
        ) {
          exist = true;
        } else if (Constants.filterLeagueApply.includes('otherLeagues')) {
          exist = primaryLeagues.includes(betslip.bets[x].event.league)
            ? exist
            : true;
        } else {
          exist;
    ...
    Constants.statsNetProfit = available
      ? betslip.netProfit + Constants.statsNetProfit
      : Constants.statsNetProfit;
    Constants.statsToRisk = available
      ? betslip.toRisk + Constants.statsToRisk
      : Constants.statsToRisk;

    (Constants.statsWins = available && betslip.outcome === 'win')
      ? 1 + Constants.statsWins
      : Constants.statsWins;
    (Constants.statsLosses = available && betslip.outcome === 'loss')
      ? 1 + Constants.statsLosses
      : Constants.statsLosses;
    (Constants.statsPushes = available && betslip.outcome === 'push')
      ? 1 + Constants.statsPushes
      : Constants.statsPushes;
...
*/
  ...
  const addStatusFilter = async item => {
    await Constants.filterStatus.push(item);
  };
      ...
      try {
        ...
        for (let x = 0; x < bets.length; x++) {
          if (Constants.filterDateApply.includes('sevenDays')) {
            days = 7;
            exist =
              new Date(bets[x].event.startTime) >=
...
*/
    ...
    Constants.statsNetProfit = betslips
      .map(item => item.netProfit)
      .reduce((a, b) => a + b, 0);

    Constants.statsAtRisk = betslips
      .map(item => item.atRisk)
      .reduce((a, b) => a + b, 0);

    Constants.statsPendingAmount = betslips
      .map(item => item.toWin)
      .reduce((a, b) => a + b, 0);
...
*/
  ...
  const filterTopLevelBetslipsApplied = betslips => {
    ...
    Constants.statsNetProfit = available
      ? betslips.netProfit + Constants.statsNetProfit
      : Constants.statsNetProfit;
    Constants.statsToRisk = available
      ? betslips.toRisk + Constants.statsToRisk
      : Constants.statsToRisk;

    (Constants.statsWins = available && betslips.outcome === 'win')
      ? 1 + Constants.statsWins
      : Constants.statsWins;
    (Constants.statsLosses = available && betslips.outcome === 'loss')
      ? 1 + Constants.statsLosses
      : Constants.statsLosses;
    (Constants.statsPushes = available && betslips.outcome === 'push')
      ? 1 + Constants.statsPushes
      : Constants.statsPushes;
...
if ( array[i] === item) { 
  ...
  const filterBetTypeApplied = x => {
    ...
    if (Constants.filterBetTypeApply.length == 0) {
      return true;
    } else if (Constants.filterBetTypeApply.includes(x.type)) {
      // checking for parlay or teaser
      return true;
    } else if (
      x.bets.length == 1 &&
      Constants.filterBetTypeApply.includes(x.bets[0].type)
    ) {
      // checking for prop
      return true;
    } else if (
      x.bets.length == 1 &&
      x.bets[0].type != 'prop' &&
      Constants.filterBetTypeApply.includes(x.bets[0].proposition)
    ) {
      // checking for total, moneyline, spread, or 3-way
      return true;
    } else if (
      x.bets.length == 1 &&
      x.bets[0].type != 'prop' &&
  ...
  const [toggleAnalysisTestModal, setToggleAnalysisTestModal] =
    ...
    <ScreenContainer style={styles.screen} scrollable={true}>
      <View style={styles.View_3b} pointerEvents={'auto'}>
        ...
        <View style={styles.ViewNU} pointerEvents={'auto'}>
          ...
          <Text style={{ color: theme.colors.error }}>{'DISPLAY'}</Text>
        </View>
      </View>
      ...
      <View pointerEvents={'auto'}>
        <Text style={{ color: theme.colors.custom_rgb0_122_255 }}>
          {'Net Profit: '}
          {netProfitSyntax(Constants['statsNetProfit'])}{' '}
        </Text>
      </View>
          ...
          return (
            <>
              <FlatList
                            ...
                            return (
                              <>
                                ...
                                >
                                  ...
                                  {listData?.line}
                                  {' ... '}
                                  {listData?.oddsAmerican}
                                </Text>
                              </>
                            );
                          }}
                          contentContainerStyle={styles.FlatListbEContent}
                          numColumns={1}
                        />
                ...
                contentContainerStyle={styles.FlatListz5Content}
                numColumns={1}
              />
            </>
          );
      ...
      <Modal
        ...
        visible={toggleAnalysisTestModal}
        animationType={'slide'}
        presentationStyle={'pageSheet'}
        transparent={true}
      >
        <View
          style={[styles.ViewHE, { backgroundColor: theme.colors.background }]}
          pointerEvents={'auto'}
        >
          <View pointerEvents={'auto'}>
            <Text style={{ color: theme.colors.error }}>
              {'filterLeague: '}
              {Constants['filterLeague'].toString()}
            </Text>

            <Text style={{ color: theme.colors.good }}>
              {'filterLeagueApply: '}
              {Constants['filterLeagueApply'].toString()}
            </Text>
          </View>
          ...
          <ScrollView showsVerticalScrollIndicator={true} bounces={true}>
            <View pointerEvents={'auto'}>
              ...
              <View pointerEvents={'auto'}>
                <Text style={{ color: theme.colors.error }}>
                  {'filterDate: '}
                  {Constants['filterDate'].toString()}
                </Text>

                <Text style={{ color: theme.colors.good }}>
                  {'filterDateApply: '}
                  {Constants['filterDateApply'].toString()}
                </Text>
                ...
                <View style={styles.ViewyV} pointerEvents={'auto'}>
                  <DatePicker
                    onDateChange={newDatePickerStartValue => {
                      const dateStart = newDatePickerStartValue;
                      try {
                  ...
                  />
                  <DatePicker
                    onDateChange={newDatePickerEndValue => {
                      const dateEnd = newDatePickerEndValue;
                      try {
                        setDateEnd(dateEnd);
                      } catch (err) {
                        console.error(err);
                    ...
                    type={'solid'}
                  />
                  <View style={{ borderRadius: 6 }} pointerEvents={'auto'} />
                </View>
              </View>
              ...
              <View pointerEvents={'auto'}>
                <Text style={{ color: theme.colors.error }}>
                  {'filterSportsbook: '}
                  {Constants['filterSportsbook'].toString()}
                </Text>

                <Text style={{ color: theme.colors.good }}>
                  {'filterSportsbookApply: '}
                  {Constants['filterSportsbookApply'].toString()}
                </Text>
                  ...
                  {({ loading, error, data, refetchGetBankrollPageById }) => {
                    ...
                    return (
                      <FlatList
                        ...
                        contentContainerStyle={styles.FlatListyUContent}
                        numColumns={1}
                      />
                    );
                  }}
              ...
              <View pointerEvents={'auto'}>
                <View pointerEvents={'auto'}>
                  <Text style={{ color: theme.colors.error }}>
                    {'filterBetType: '}
                    {Constants['filterBetType'].toString()}
                  </Text>

                  <Text style={{ color: theme.colors.good }}>
                    {'filterBetTypeApply: '}
                    {Constants['filterBetTypeApply'].toString()}
                  </Text>
                </View>
              ...
              <View pointerEvents={'auto'}>
                <View pointerEvents={'auto'}>
                  <Text style={{ color: theme.colors.error }}>
                    {'filterUnderdog: '}
                    {Constants['filterUnderdog'].toString()}
                  </Text>

                  <Text style={{ color: theme.colors.good }}>
                    {'filterUnderdogApply: '}
                    {Constants['filterUnderdogApply'].toString()}
                  </Text>
                </View>
                ...
                <View style={styles.ViewBo} pointerEvents={'auto'}>
                  <Touchable style={styles.TouchableWx}>
                    ...
                    >
                      <Text
                        style={[styles.Text_03, { color: theme.colors.strong }]}
                      >
                        {'All'}
                      </Text>
                    </View>
                  ...
                  <Touchable style={styles.Touchableie}>
                    ...
                    >
                      <Text
                        style={[styles.TextH4, { color: theme.colors.strong }]}
                      >
                        {'Favorites'}
                      </Text>
                    </View>
              ...
              <View pointerEvents={'auto'}>
                <View pointerEvents={'auto'}>
                  <Text style={{ color: theme.colors.error }}>
                    {'filterUnder: '}
                    {Constants['filterUnder'].toString()}
                  </Text>

                  <Text style={{ color: theme.colors.good }}>
                    {'filterUnderApply: '}
                    {Constants['filterUnderApply'].toString()}
                  </Text>
                </View>
                ...
                <View style={styles.ViewE0} pointerEvents={'auto'}>
                  <Touchable style={styles.TouchableDS}>
                    ...
                    >
                      <Text
                        style={[styles.TextFe, { color: theme.colors.strong }]}
                      >
                        {'All'}
                      </Text>
                    </View>
                  ...
                  <Touchable style={styles.TouchableWb}>
                    ...
                    >
                      <Text
                        style={[styles.TextfJ, { color: theme.colors.strong }]}
                      >
                        {'Overs'}
                      </Text>
                    </View>
              ...
              <View pointerEvents={'auto'}>
                <View pointerEvents={'auto'}>
                  <Text style={{ color: theme.colors.error }}>
                    {'filterOutcome: '}
                    {Constants['filterOutcome'].toString()}
                  </Text>

                  <Text style={{ color: theme.colors.good }}>
                    {'filterOutcomeApply: '}
                    {Constants['filterOutcomeApply'].toString()}
                  </Text>
                </View>
                ...
                <View style={styles.Viewax} pointerEvents={'auto'}>
                  <Touchable style={styles.TouchableTd}>
                    ...
                    >
                      <Text
                        style={[styles.TextwQ, { color: theme.colors.strong }]}
                      >
                        {'All'}
                      </Text>
                    </View>
                  ...
                  >
                    ...
                    >
                      <Text
                        style={[styles.TextOr, { color: theme.colors.strong }]}
                      >
                        {'Losses'}
                      </Text>
                    </View>
              ...
              <View pointerEvents={'auto'}>
                <View pointerEvents={'auto'}>
                  <Text style={{ color: theme.colors.error }}>
                    {'filterBonus: '}
                    {Constants['filterBonus'].toString()}
                  </Text>

                  <Text style={{ color: theme.colors.good }}>
                    {'filterBonusApply: '}
                    {Constants['filterBonusApply'].toString()}
                  </Text>
                </View>
                ...
                <View style={styles.View_39} pointerEvents={'auto'}>
                  ...
                  <Touchable style={styles.TouchableXX}>
                    ...
                    >
                      <Text
                        style={[styles.TextW3, { color: theme.colors.strong }]}
                      >
                        {'Bonuses Only'}
                      </Text>
                    </View>
                  ...
                  <Touchable style={styles.TouchableYn}>
                    ...
                    >
                      <Text
                        style={[styles.TexthQ, { color: theme.colors.strong }]}
                      >
                        {'No Bonuses'}
                      </Text>
                    </View>
              ...
              <View pointerEvents={'auto'}>
                <View pointerEvents={'auto'}>
                  <Text style={{ color: theme.colors.error }}>
                    {'filterLiveBet: '}
                    {Constants['filterLiveBet'].toString()}
                  </Text>

                  <Text style={{ color: theme.colors.good }}>
                    {'filterLiveBetApply: '}
                    {Constants['filterLiveBetApply'].toString()}
                  </Text>
                </View>
                ...
                <View style={styles.ViewY3} pointerEvents={'auto'}>
                  <Touchable style={styles.TouchableaY}>
                    ...
                    >
                      <Text
                        style={[styles.TextVU, { color: theme.colors.strong }]}
                      >
                        {'All'}
                      </Text>
                    </View>
                  ...
                  <Touchable style={styles.TouchableIf}>
                    ...
                    >
                      <Text
                        style={[styles.Textlv, { color: theme.colors.strong }]}
                      >
                        {'All'}
                      </Text>
                    </View>
                  ...
                  <Touchable style={styles.Touchablele}>
                    ...
                    >
                      <Text
                        style={[styles.TextVV, { color: theme.colors.strong }]}
                      >
                        {'All'}
                      </Text>
                    </View>
