...
import { ButtonOutline, Divider, Icon, ScreenContainer, Touchable, withTheme } from '@draftbit/ui';
...
const [textInputValue, setTextInputValue] = React.useState("");
  ...
  } style={styles.ButtonOutlineCw} title={"Totals"} /><ButtonOutline onPress={
     () => { 
      try {
          setGlobalVariableValue({
          key: "oddsDisplayed",
  ...
  } style={styles.ButtonOutlinekI} title={"Spreads"} /><ButtonOutline onPress={
     () => { 
      try {
          emptySearchBar();
      } catch (err) {
  ...
  } style={styles.ButtonOutlineHE} title={"Clear Search Bar"} /><TextInput onChangeText={
  ...
  } style={[styles.TextInputUy, { backgroundColor: theme.colors.background_inverse }]} placeholder={"Enter a value..."} value={textInputValue} clearButtonMode={"while-editing"} autoCapitalize={"words"} />
      </View>
 ...
 >
  {({ loading, error, data, refetchOddsData }) => {
    ...
    return (
      ...
      <View  style={[styles.ViewwW, { backgroundColor: theme.colors.divider }]} pointerEvents={"auto"}>
        
      <Text  style={[styles.TextRH, { color: theme.colors.good }]} >
        {listData?.awayTeam}
      </Text>
    
      <Text  style={[styles.Text_2J, { color: theme.colors.good }]} >
        {listData?.homeTeam}
      </Text>
    <Divider  style={styles.Dividerw9} color={theme.colors.error} />
      </View>
    ) }}  contentContainerStyle={styles.FlatListz7Content} numColumns={1} />
      <ScrollView   showsVerticalScrollIndicator={false} bounces={false} horizontal={true} showsHorizontalScrollIndicator={false}>
        <Utils.CustomCodeErrorBoundary><CustomCode.GamesScrollList data={filterList(fetchData)} /></Utils.CustomCodeErrorBoundary>
      </ScrollView>
...
const styles = StyleSheet.create({ ImageeQ: {
width: 12,
height: 12,
}, Imaget5: {
width: 12,
